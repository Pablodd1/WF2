...
const { loadAppliedDealerIds, profileWithLinkageState } = require('./_lib/dealer-linkage-state.cjs');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!canonicalError && canonicalProfile?.dealer) {
      const appliedDealerIds = await loadAppliedDealerIds(client, [canonicalProfile.dealer.id]);
      const hasAppliedLinks = appliedDealerIds.has(canonicalProfile.dealer.id);
      return res.status(200).json({
        success: true,
        ...profileWithLinkageState(canonicalProfile, hasAppliedLinks),
