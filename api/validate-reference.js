 ...
 * Falls back to web search if not in local catalog
 */

import { createClient } from '@supabase/supabase-js';

// Known reference catalog (seeded from current data + known models)
const KNOWN_REFS = new Set([
    ...
    '116519', '116506', '116505', '116508', '126529', '126622',
...
// Extract canonical ref from parsed reference
function normalizeRef(ref) {
    if (!ref) return null;
    ref = ref.toUpperCase().trim();
    // Remove trailing slashes/dashes
    ref = ref.replace(/[\/\-]$/, '');
    return ref;
}
...
export default function handler(req, res) {
    ...
    const references = req.method === 'POST' 
        ? (Array.isArray(req.body) ? req.body : [req.body.reference]).filter(Boolean)
        : [req.query.ref].filter(Boolean);
    ...
    const results = references.map(ref => {
        ...
        const isKnown = KNOWN_REFS.has(normalized);
        
        // Also check partial matches (e.g., 5712/1A-001 matches 5712/1A)
        const partialMatch = !isKnown && Array.from(KNOWN_REFS).some(known => 
            normalized && (normalized.startsWith(known) || known.startsWith(normalized))
        );
        ...
        return {
            ...
            note: isKnown ? 'Verified in catalog' : partialMatch ? 'Partial match' : 'Unknown reference - verify against brand website',
        };
    });
