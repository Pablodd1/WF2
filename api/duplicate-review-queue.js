...
module.exports = async function handler(req, res) {
  ...
  try {
    const result = await rest(baseUrl, key, `duplicate_review_candidates?${params.toString()}`);
    ...
    return res.status(200).json({ status: 'ok', page, limit, total: result.total, queueStatus: status, items });
  } catch (error) {
