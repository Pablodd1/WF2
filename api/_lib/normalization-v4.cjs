...
const CURRENCY_ALIASES = [
  ...
  { code: 'HKD', pattern: 'HKD|HDK|HK\\$|H\\.?K\\.?D\\.?|H\\.?D\\.?K\\.?|港幣|港币|港元|萬港幣|万港币|萬港币|万港幣' },
...
function normalizeCurrencyToken(token) {
  ...
  if (
    /^(HKD|HDK|HK\$|H\.?K\.?D\.?|H\.?D\.?K\.?)$/.test(clean)
    || /港币|港幣|港元|萬港幣|万港币|萬港币|万港幣/.test(token)
  ) return 'HKD';
...
function extractPriceObservations(text, context = {}) {
  ...
  const multipliers = '(k|m|mn|w|mil|mill|ten|thou|thousands|萬|万)';
  const leftCurrency = new RegExp(`(${CURRENCY_TOKEN})\\s*([\\d][\\d.,]*)(?:\\s*${multipliers})?`, 'gi');
  const rightCurrency = new RegExp(`([\\d][\\d.,]*)(?:\\s*${multipliers})?\\s*(${CURRENCY_TOKEN})`, 'gi');
  ...
  const dollarPattern = /\$\s*([\d][\d.,]*)(?:\s*(k|m|mn|w|mil|mill|ten|thou|thousands))?/gi;
  ...
  if (!observations.length && context.currency_context) {
    const bare = line.match(/\b(\d{1,3}(?:[.,]\d{3})+|\d+(?:[.,]\d+)?)\s*(k|m|mn|w|mil|mill|ten|thou|thousands)\b/i);
