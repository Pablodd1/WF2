-- Direct bounded customer rows for broad two-brand pages. PL/pgSQL resolves
-- the enabled run first so the ordered staging scan receives a concrete index
-- prefix and cannot be flattened back into the expensive release-view join.
