-- Keep usable customer prices ahead of no-price activity on broad Rolex/Patek
-- Trading Floor pages. No-price WTS/WTB records remain published and reachable
-- through the same cursor; they are only placed after priced observations.

CREATE INDEX IF NOT EXISTS idx_staging_qnsa_brand_price_first_feed
  ON staging.listings (
    brand_normalized,
    ((price_usd >= 1000 AND (
      currency_normalized IN ('USD', 'USDT') OR
      (conversion_rate > 0 AND conversion_timestamp IS NOT NULL)
    ))) DESC,
    created_at DESC,
    id DESC
  )
...
CREATE INDEX IF NOT EXISTS idx_staging_qnsa_brand_intent_price_first_feed
  ON staging.listings (
    brand_normalized,
...
-- Reinstall the bounded function after the indexes exist, changing only the
-- deterministic ordering in both bounded CTEs and the final projection.
...
BEGIN
  ...
  WITH candidate_ids AS MATERIALIZED (
    ...
    ORDER BY (l.price_usd >= 1000 AND (l.currency_normalized IN ('USD', 'USDT')
        OR (l.conversion_rate > 0 AND l.conversion_timestamp IS NOT NULL))) DESC NULLS LAST,
  ...
  FROM (
    SELECT
      ...
      COALESCE(NULLIF(btrim(l.user_name), ''), NULLIF(btrim(l.from_name), ''), NULLIF(btrim(rv.raw_payload#>>'{raw_data,from_name}'), '')) AS seller_name,
      COALESCE(NULLIF(btrim(l.contact_number), ''), NULLIF(btrim(l.from_number), ''), NULLIF(btrim(rv.raw_payload#>>'{raw_data,from_number}'), '')) AS seller_phone,
      (COALESCE(NULLIF(btrim(l.contact_number), ''), NULLIF(btrim(l.from_number), ''), NULLIF(btrim(rv.raw_payload#>>'{raw_data,from_number}'), '')) IS NOT NULL) AS contact_publication_approved,
      ...
      CASE WHEN l.currency_normalized IN ('USD', 'USDT') AND l.price_usd > 0 THEN 'SOURCE_EXPLICIT_USD_MATCH'
        WHEN l.price_usd > 0 AND l.conversion_rate > 0 AND l.conversion_timestamp IS NOT NULL THEN 'EXPLICIT_SOURCE_FX_CONVERTED'
        WHEN l.price_normalized > 0 THEN 'CURRENCY_UNCONFIRMED' ELSE 'PRICE_NOT_SUPPLIED' END AS price_evidence_status,
      ...
      CASE WHEN l.currency_normalized IN ('USD', 'USDT') AND l.price_usd > 0 THEN l.price_usd
        WHEN l.price_usd > 0 AND l.conversion_rate > 0 AND l.conversion_timestamp IS NOT NULL THEN l.price_usd ELSE NULL END AS verified_price_usd,
      (l.price_usd > 0 AND (l.currency_normalized IN ('USD', 'USDT') OR (l.conversion_rate > 0 AND l.conversion_timestamp IS NOT NULL))) AS has_verified_usd_price,
      ...
      COALESCE(l.dealer_rating, l.rating, CASE WHEN COALESCE(rv.raw_payload#>>'{raw_data,dealer_rating}', '') ~ '^[0-9]+([.][0-9]+)?$'
...
REVOKE ALL ON FUNCTION public.qnsa_trading_floor_page_rows(TEXT, INTEGER, INTEGER, TEXT) FROM PUBLIC;
