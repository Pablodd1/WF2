-- Stable, bounded pagination for the reviewed Richard Mille and Cartier feeds.
--
-- The prior wrapper applied its reference-shape gate after the base function's
-- LIMIT/OFFSET window.  A page could therefore return fewer than p_limit rows
-- even though later eligible rows existed.  Advancing by the returned count
-- then re-read part of the same source window, repeating IDs across pages and
-- terminating the broad RM feed before all exact-reference rows were reachable.
--
-- This replacement applies every publication and reference gate before the
-- single deterministic LIMIT/OFFSET.  Output remains capped at 51 rows and the
-- existing (brand_normalized, reference_normalized) access path remains usable.
...
BEGIN
  ...
  WITH eligible_ids AS MATERIALIZED (
    ...
    CROSS JOIN LATERAL (
      SELECT regexp_replace(
        upper(COALESCE(l.reference_normalized, '')),
        '[^A-Z0-9]', '', 'g'
      ) AS reference_key
    ) AS normalized
    WHERE l.normalization_run_key = v_run_key
      ...
      AND (
        (p_brand = 'Richard Mille' AND normalized.reference_key ~ '^RM[0-9]{3,6}[A-Z]{0,3}$')
  ...
  FROM (
    SELECT
      ...
      'QNSA_REVIEWED_LATER_BRAND_V2'::text AS publication_lane,
