    ...
    <section aria-label="Featured reference shortcuts" className="mt-4">
      <div className="mb-2 flex flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <p className={`mt-1 text-xs ${light ? 'text-[#675b4d]' : 'text-white/60'}`}>
            {mode === 'trading'
        ...
        {PRIORITY_REFERENCE_COHORTS.map(cohort => {
          ...
          return (
            <button
              ...
              style={{
                borderColor: isActive ? '#9a7127' : light ? '#d8cfbf' : 'rgba(255,255,255,0.18)',
                background: isActive ? 'rgba(154,113,39,0.14)' : light ? 'rgba(255,255,255,0.46)' : 'rgba(255,255,255,0.06)',
                color: light ? '#211b15' : '#FFFFFF',
              }}
            >
              <span className="min-w-0">
                <span className="block truncate text-sm font-semibold">{cohort.label}</span>
                <span className={`mt-1 block text-[11px] ${light ? 'text-[#766857]' : 'text-white/55'}`}>{cohort.scope}</span>
              </span>
